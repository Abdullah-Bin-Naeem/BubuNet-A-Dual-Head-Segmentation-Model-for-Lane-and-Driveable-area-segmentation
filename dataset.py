...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, transform=None,valid=False):
        ...
        if valid:
            self.root=r'/home/abdullah/Documents/expADAS/bdd100k/images/val'
            self.names=os.listdir(self.root)
        else:
            self.root=r'/home/abdullah/Documents/expADAS/bdd100k/images/train'
            self.names=os.listdir(self.root)
    ...
    def __getitem__(self, idx):
        ...
        seg_path = image_name.replace("images","seg").replace("jpg","png")
        lane_path = image_name.replace("images","lanes").replace("jpg","png")
...
def visualize_batch(image_names, images, masks, num_samples=4):
    """
    Visualize a batch of images and their drivable area and lane masks.
    
    Args:
        image_names: List of image filenames.
        images: Tensor of shape [batch_size, 3, H, W].
        masks: Tuple of (seg_da, seg_ll), each of shape [batch_size, 2, H, W].
    ...
    seg_da = seg_da.argmax(dim=1).cpu().numpy()  # [batch_size, H, W]
    seg_ll = seg_ll.argmax(dim=1).cpu().numpy()  # [batch_size, H, W]
    images = images.permute(0, 2, 3, 1).cpu().numpy()  # [batch_size, H, W, 3]
    ...
    for i in range(num_samples):
        ...
        # Drivable Area Mask
        axes[i][1].imshow(seg_da[i], cmap='gray')
        axes[i][1].set_title("Drivable Area Mask")
        axes[i][1].axis('off')

        # Lane Mask
        axes[i][2].imshow(seg_ll[i], cmap='gray')
        axes[i][2].set_title("Lane Mask")
        axes[i][2].axis('off')
